...
- **Single-system first-class.** Install → open → bill on one PC with zero network configuration. LAN clients are additive; Settings shows the server's LAN URL + QR code for connecting devices.
...
6. **Resilience + packaging** — draft queue polish, backups, watchdog, Windows installer (firewall rule, autostart), LAN QR connect screen.
